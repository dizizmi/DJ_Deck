...
DeckGUI::DeckGUI(DJAudioPlayer* _player,
    ...
    : player(_player), waveformsDisplay(formatManagerToUse,cacheToUse)


{
    //loading pause and play button images
    auto pauseImage = juce::ImageCache::getFromMemory(BinaryData::pause_button_png, BinaryData::pause_button_pngSize);
    ...
    addAndMakeVisible(waveformsDisplay);
...
void DeckGUI::resized()
{
    ...
    waveformsDisplay.setBounds(0, rowH * 4, getWidth(), rowH*3);
...
void DeckGUI::buttonClicked(juce::Button* button) {
    ...
    if (button == &loadButton) { //If button is clicked, allows user to select file to be loaded into the deck
        ...
        waveformsDisplay.loadURL(juce::URL{chooser.getResult()});
...
void DeckGUI::timerCallback() { //Timer loops every 100ms, which checks if the "loop" button is on or off and also updates the wave form display current position
    ...
    waveformsDisplay.setPositionRelative(player->getPositionRelative()); //Every 100ms, updates the waveform display to show current timestamp for the audio file
}
...
void DeckGUI::playlistToDeckGUI() { //function rea
        ...
        waveformsDisplay.loadURL(audioURL); //Loads the file and returns the waveform of the audio file
